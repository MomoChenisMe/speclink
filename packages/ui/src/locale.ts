// UI 語言偏好：localStorage 單鍵，null 表跟隨系統；與 config.yaml 的 locale
//（AI artifacts 產出語言）互不影響。
//
// 這四個函式與哪個 app 無關，Desktop 與 Server 兩端都要用——抄第二份就是這類工具漂移
// 的起點，所以放在共用套件，Desktop 的 i18n/locale.ts 改為自此 re-export。
