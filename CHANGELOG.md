...
- `speclink plan` 的波次改成只看宣告的依賴，兩個變更動到同一個 capability 不再把其中一個推到後面，沒有依賴的小變更可以馬上開工；兩個變更動到同一個 requirement 時，plan 改為標出封存順序（誰要等誰先封存）與同名衝突，archive 與 commit 技能封存前會提醒、封存後會點名排在後面的變更先對照正式規格重寫；沒有看板順序的變更改依被依賴數、任務數與建立時間排，新增的 `speclink change rank` 可以把變更排到另一個之前或之後，propose 與 ingest 收尾會替小而急的變更插隊；要用舊算法時加 `--strict-overlap`（只限本機）。
- 桌面 app 詳情面板的「排程」分頁改成波次、前置、重疊、封存順序四張卡片：是否被擋住改成前置卡上的徽章，重疊卡列出和哪個變更撞到哪個 requirement、雙方各做了什麼操作，封存順序卡列出封存時要排在誰後面，遇到同名衝突或互相等待時顯示警示；remote 專案也看得到同一份資訊，連到舊版 server 時前置卡會另列 server 依重疊判定的等待。
