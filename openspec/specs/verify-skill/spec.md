...
技能 SHALL 於主線接手 fork 報告後進行 remediation triage；修正一律回主線依專案 TDD 慣例執行，fork 不得修改任何檔案。Discovery 有 findings 時 SHALL 詢問三選項——修正後重驗／接受現狀蓋章（`verify stamp --accept`）／先不蓋章結束；零 findings 時 SHALL 記錄空 discovery round 並由主線執行 `verify stamp`。
...
- **WHEN** discovery 的三維度皆零 findings
