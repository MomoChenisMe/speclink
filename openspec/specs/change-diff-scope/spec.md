...
`--json` resolved payload SHALL 含 change:string、phase:"discovery"|"validation"、state:"resolved"、baseCommit:string、candidateHash:string、patchHash:string、paths:string[]、files:array、patch:string；files 每項 SHALL 含 oldPath:string|null、newPath:string|null、kind:string、beforeHash:string|null、afterHash:string|null、hunks:array；hunks 每項 SHALL 含 id:string 與四個 number ranges。human 成功路徑 SHALL 將 phase、patchHash、路徑數與 hunk 數寫至 stdout。兩條路徑於 `--no-color` 下 SHALL 不含 ANSI。
...
baseline 缺失／late／unavailable、base commit 不可解析、touched path 已在 dirtyFilesAtStart、另一 active change 的 touched record 重疊、remediation snapshot 缺失或先前已髒且未保存的檔案再次改變時，review scope SHALL 視為 needsInput，不得建立 frozen snapshot。
...
工單不存在時 phase SHALL 為 discovery。工單存在時 phase SHALL 為 validation，resolver SHALL 只比較上輪未解 finding paths 的 frozen afterText 與目前內容，再加入上輪 capture 後才變髒的新路徑；上輪已髒、未在保存 scope 且內容又改變的路徑 SHALL needsInput。snapshot 缺失或工單 patchHash 與 snapshot 不符 SHALL 非零結束，不得退回整檔 discovery。
