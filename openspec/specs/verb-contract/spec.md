...
- **FsOnly**（demo、trace、plan、change）：remote 模式 SHALL 以非零 exit code 明確拒絕，拒絕判定 SHALL 僅解析模式而不建立連線——SHALL NOT 發出任何 server 請求，離線環境同樣拒絕。plan 與 change 的 remote 臂留待後續變更補齊後轉為 Dual。
...
- **WHEN** 於 remote 模式設定且 server 不可達的環境下執行 speclink demo、speclink plan 與 speclink change depends
- **THEN** 三者 exit code 非零，stderr 說明該動詞僅限本機模式，且過程未發出任何 server 請求
