...
技能 SHALL 只承載兩站時序,SHALL NOT 重述兩站的檢查內容、工單與蓋章語意(由 review/verify 技能正典各自承載)。前提為 change 寫碼任務全數完成(非 `[M]` 任務全數勾選;僅餘 `[M]` 手動測試任務未勾時前提成立,技能於報告點名尚餘手測);寫碼任務未完成時 SHALL 依兩站既有前提行為呈現(拒絕或中途盤點,依該站正典),技能不另設守門。
...
- 收尾:使用者裁示補蓋後,review 章與 verify 章接連補蓋;補蓋時技能 SHALL 向兩站明示本次為收尾補蓋呼叫——兩站據此關閉其禁蓋例外;未宣告即補蓋不合本規格。票內仍留有使用者裁示不修的必修級 findings 時,該站之章為帶保留章——技能 SHALL 於補蓋選項中預先載明,並以停下時的使用者裁示為明示授權;SHALL NOT 未經明示逕自蓋保留章。僅 SUGGESTION 級殘留 SHALL NOT 構成保留——該站收尾補蓋落乾淨章,無需保留授權。兩章 SHALL 接連落、中間零編輯,使兩章至封存皆不出現「其後有變動」狀態。封存 SHALL 以建議形式提出、由使用者執行;change 尚餘 `[M]` 任務未勾時,封存建議 SHALL 提醒手動測試完成並勾選後才可封存。

#### Scenario: 完整兩站時序

- **WHEN** 使用者對寫碼任務全數完成的 change 要求兩站都跑(或執行 /speclink-quality)
...
- **THEN** 技能 SHALL NOT 重做已蓋章的站,照跑新站;新站各輪零碼改動時前章維持有效、封存照行;新站的修正落地時前章即失效,封存的章失效守門拒絕至前章之站重跑補章(見 change-lifecycle「封存的章失效守門」)——重跑即「蓋章後再修碼」的代價,兩站已知都要跑時走本技能的雙章保留時序即可避免
...
- **WHEN** change 尚有未完成寫碼任務即啟動兩站時序
- **THEN** 依兩站既有前提行為呈現(review 站拒絕並停止;verify 站以中途盤點報告收場),quality 不另設守門、不吞錯

#### Scenario: 僅餘手動任務時兩站照跑

- **WHEN** 對寫碼任務全勾、僅餘 `[M]` 任務未勾的 change 執行 /speclink-quality
- **THEN** 兩站時序照常啟動,報告點名尚餘的手動測試任務;收尾補蓋照常可行,封存建議提醒手測完成後才可封存
