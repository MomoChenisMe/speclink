...
引擎改經儲存介面存取規格文件後，CLI 的所有既有指令 SHALL 維持與重構前完全一致的可觀察行為：人眼輸出（含色彩與 --no-color）、`--json` payload 的欄位（camelCase）與值、exit code、以及對檔案系統的效果。本需求為 parity 敏感：對 Spectra 2.3.1 的一致性基線 SHALL 維持位元級不變（parity_suite 31 項、color_suite 16 項、twin harness 8 情境為驗證載體）。
