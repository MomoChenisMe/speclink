...
任務清單呈現 SHALL 剝離 speclink-task 註解(使用者可見文字與無註解時相同),並 SHALL 剝離行首標記前綴(`[M]`/`[P]`,順序不敏感、各至多一次)——`[M]` 的身分 SHALL 以獨立旗標供呈現層使用(徽章呈現歸 desktop-app 正式規格),`[P]` 剝離後 SHALL NOT 留下任何呈現;清單項 SHALL 以 stable ID 作呈現 key(無 ID 舊檔退回 ordinal key);勾選操作 SHALL 以 stable ID 定址(無 ID 任務走 ordinal 相容路徑);樂觀就地改寫 SHALL 保留行首前綴與行尾註解原文。
