...
任務行 [M] 手動任務標記的語意：標記的解析與顯示剝離、把任務分成寫碼與手動兩類後的寫碼完成度預測子，以及任務 payload 的 manual 欄位。標記涵蓋一切代理無法代行、需使用者親手操作的任務，不限於測試。本 capability 保證代理不會替使用者勾掉自己做不到也無法觀察的項目——apply 在寫碼任務全數完成時即回報完成，把手動項明列給使用者親自執行，並在寫碼任務被未勾的手動項擋住時停下請使用者先行。
