...
- **THEN** stdout、stderr 與 exit code 逐位元一致；`crates/adapters/speclink-cli/tests/` 的整合測試（含 `--no-color` 人眼輸出斷言與 fs／remote 對照）與 `crates/engine/speclink-core/tests/render_golden.rs` 全綠
