...
<!-- @trace
source: discussion-last-cut-release
updated: 2026-09-12T22:38:53+08:00
-->
...
server SHALL 提供 POST /changes/{name}/depends，body 為 { on: string[], remove: boolean }，經 Command gateway 直通引擎的依賴寫入命令，editor role 限定（reader 回 403）。change 不存在 SHALL 回 404。自依賴、任一 on 名稱不存在或已封存（非 remove 時）、非 remove 且加邊後成環 SHALL 回 HTTP 409、reason 為 refused（沿用封閉 error reason registry，SHALL NOT 新增 reason 值），message SHALL 為引擎原文，逐字與 fs 模式 `speclink change depends` 的 stderr 相同：自依賴為 `'<name>' cannot depend on itself`、不存在為 `cannot depend on '<x>': no active change with that name`、已封存為 `cannot depend on '<x>': it is already archived`、成環為 `dependency cycle: <name> -> … -> <name>`。守門失敗 SHALL 零寫入。實際改動 meta 時 SHALL 發布領域事件、scope revision 前進並發布 invalidate；冪等無改動時 SHALL HTTP 200、零寫入、零事件、revision 不前進。回應 SHALL 為 { change: string, dependsOn: string[] }（寫入後完整清單）附 scope ETag。
...
- **WHEN** editor 呼叫 POST /changes/ghost/depends，scope 內沒有 ghost
- **THEN** HTTP 404，reason 為 not_found，revision 不前進

#### Scenario: 守門失敗 409

- **WHEN** add-a 的 depends_on 含 add-b，old-change 已封存，editor 分別以 on ["add-b"] 呼叫 POST /changes/add-b/depends、以 on ["old-change"] 呼叫同一端點、以 on ["add-a"] 呼叫同一端點
- **THEN** 三次皆 HTTP 409、reason 皆為 refused，message 依序為 `'add-b' cannot depend on itself`、`cannot depend on 'old-change': it is already archived`、`dependency cycle: add-b -> add-a -> add-b`，meta 皆不變、revision 不前進
