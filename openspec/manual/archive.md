...
section: SDD 工作流
order: 170
keywords: [封存, archive, 正式規格, 守門, Purpose, 證據, 手冊, 封存順序, 同名衝突]
...
技能在執行封存指令之前，先執行 `speclink plan --json`，看三件事：

| plan 顯示 | 技能的提醒 |
| --- | --- |
| 目標封存要等別的變更（兩者動到同名 requirement，對方要先封存） | 「plan 建議先封存 <那些變更>；它們封存後，重讀本 change 對同名 requirement 的 MODIFIED／REMOVED／RENAMED 區塊、對照正式規格重寫（走 ingest）再封存」，並列出重疊的 capability 與 requirement 名 |
| 目標與別的變更衝突（都新增或改名成同一個名稱，或都移除或改名掉同一個名稱） | 「與 <變更> 都新增（或改名成）同名的 requirement <名稱>，或都移除（或改名掉）它；後封存的那一個會封存失敗（名字重複，或名字已不存在），先改掉其中一邊」 |
| 目標的宣告前置還沒封存 | 「宣告前置 <名稱> 尚未封存」 |

三者都只是建議：技能不會擋下封存，你確認後照常封存。技能不會叫你「跑 drift 就好」：drift 只查 requirement 名在不在，抓不到內文被蓋掉。三者都沒有、目標不在 plan 裡、或 plan 失敗（依賴成環）時，技能不提順序。

技能同時記下「封存要等目標」的其他變更：它們排在目標之後，動到同名 requirement。目標封存成功後，技能會點名它們（見下面的「封存之後」）。

沒指名變更時，技能列的候選清單也來自 plan：依 plan 的配置順序排列，每個候選標出能不能封存（封存不必等別人就標「可封存」，要等就標「等 <名稱> 封存」）；宣告前置還沒封存時，再標「前置 <名稱> 未封存」。要封存哪一個仍由你選，技能不會自動選。plan 失敗時，候選清單退回 `speclink list` 的順序，不標阻擋。

| plan 的內容 | 候選清單 |
| --- | --- |
| add-a 封存不必等別人、add-b 封存要等 add-a，配置順序 add-a、add-b | add-a 標「可封存」、add-b 標「等 add-a 封存」，由你選 |
| plan 回報依賴成環 | 改用 `speclink list` 的順序，不標阻擋 |

封存順序與衝突怎麼算，見[執行順序：plan 與依賴](plan.md)。
...
- **排在後面的變更要先重寫**：封存前記下的「封存要等目標」的變更不是空的時，技能提醒它們先重讀同名 requirement 的區塊、對照正式規格重寫（走 ingest）再封存。目標封存後 plan 不再列出它，這是最後一次點名的機會。這一條不會另外問你問題，也不會代跑 ingest。
