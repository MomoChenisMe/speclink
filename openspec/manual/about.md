...
- 來源規格改了、但改的部分與某一頁的內容無關時，那一頁重生後內文不變，只換生成時戳，「可能過期」標記就消掉。本次 `desktop-app` 的三頁（[認識桌面 app](desktop-overview.md)、[規格、討論、已封存與搜尋](desktop-browse.md)、[桌面上的品質關卡](desktop-quality.md)）從裸名改成錨定寫法，各自只錨定它取材的需求段。
