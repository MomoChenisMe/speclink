...
- **definition**: 寫碼任務全部完成、僅餘未勾的手動測試任務未完成的變更狀態——看板卡片以行內小章標示,tooltip 載明剩餘項數。這是「輪到使用者了」的訊號,與「寫碼寫到一半」的同分數進度明確區分。
- **avoid**: 等待驗收、待人工、待手動測試、awaiting manual(中文散文中)
- **why**: 三字短語塞得進卡片 tooltip,且與「手動測試」同詞根;「等待驗收」聽起來像流程關卡,實際只是還有幾個手測任務沒勾。2026-08-11 討論「task-marker-ui-and-parallel-removal」定案。
