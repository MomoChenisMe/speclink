...
引擎解析討論記錄時 SHALL 只把整行為「## Context」「## Rounds」「## Conclusion」的行視為結構標題；輪內文中其他以「## 」開頭的行 SHALL NOT 截斷區段、SHALL NOT 改變 add-round 的插入點。討論內容寫入動詞（context、add-round、conclude）SHALL 在落盤前把撞名內容行（整行為結構標題，或行首為「### Round 」「## Round 」前綴）以 markdown 反斜線跳脫。輪計數 SHALL 只認合法輪標題形狀，SHALL NOT 因內文撞名而膨脹或跳號。桌面與 Web 的區段切分 SHALL 與引擎採同一結構標題白名單。
