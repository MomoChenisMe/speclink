...
限制：技能 asset 改動連動 ASSET_VERSION、render golden 五份快照與 assets.lock；既有 CLI 指令輸出與討論記錄格式不得變；正典 verb-contract「技能資產不含直接讀檔指示」禁止技能檔指示直接開啟規格目錄的檔案路徑（守門：speclink-core 的 skill_verbization 測試），文件閱讀一律以 speclink 動詞表述；同期進行中變更 add-plan-handoff-after-archive 也升 ASSET_VERSION（排定 v1.36.0 → v1.37.0），兩者合併時版號行會對撞。
...
### D1 delta 命中以 speclink show 的 deltaSpecs 比對，不加引擎輸出

技能檔規定：Canon pass 命中 capability 名後，對 speclink list --json 回傳的每個進行中變更執行 speclink show <name> --json，只取其 deltaSpecs 欄位（`<capability>/spec.md` 條目），與命中的 capability 名取交集；交集即「進行中 delta 命中」，變更名即該次 show 的 <name>。理由：deltaSpecs 是 show --json 既有的契約欄位，fs 與 remote 兩模式同形（protocol 的 deltaCapabilities），零引擎改動；正典 verb-contract「技能資產不含直接讀檔指示」禁止技能檔指示直接開啟規格目錄的檔案路徑，skill_verbization 守門測試以「read／glob／open 同一行接規格目錄路徑」的正規式把這條紀律變成紅燈。替代方案「以 Glob 比對 openspec/changes/*/specs/<capability>/spec.md」被否決：apply 期實測踩到 skill_verbization 守門，且 remote 模式沒有本機目錄可比。替代方案「引擎 list --json 加 capability 欄位」被否決：改 JSON 契約、動 protocol／server／remote 三層，而 show --json 已帶同一資訊。替代方案「技能檔自己讀每個變更的 proposal Impact 段」被否決：Impact 是散文，比對不穩且讀量大。

已知邊界：show --json 的 payload 同時帶 proposal／design／tasks 全文，技能檔規定只取 deltaSpecs 欄位；每個進行中變更一次呼叫，變更數多時呼叫次數線性成長（接受：偵察仍是淺掃，進行中變更通常個位數）。remote 模式走同一組動詞，不另闢查詢路徑。

### D2 時間盒：只讀標題與標記，至多 3 份

delta 命中以 speclink artifact cat specs/<capability> --change <name> 取內容，只保留 Requirement 標題（`### Requirement:` 行）與 ADDED／MODIFIED／REMOVED／RENAMED 區段標記，不讀全文，至多 3 份（超過取正式規格命中順序前 3 個 capability 的 delta）；artifact cat 為 Dual 動詞，remote 模式同樣可用。理由：discuss-skill 規格「開場 scout 維持淺掃」要求偵察為接地與需求清晰度判定，不是調查；深入查證沿樹逐節點進行。替代方案「delta 全文讀」被否決：一份 delta 常有數百行，三份就超過原始碼 5 檔的量。
...
- Step 2 Canon pass 段落規定：命中 capability 名後對 speclink list --json 回傳的每個進行中變更執行 speclink show <name> --json、只取 deltaSpecs 欄位比對；含該 capability 者列為「進行中 delta 命中」並記變更名；以 speclink artifact cat specs/<capability> --change <name> 只讀 Requirement 標題與 ADDED／MODIFIED／REMOVED／RENAMED 標記、至多 3 份；正式規格零命中或沒有進行中變更時靜默跳過；技能文字不含直接開啟 openspec/changes/ 路徑的讀檔指示（skill_verbization 守門綠）。
...
**具體例**：題目關鍵字命中正式規格 client-protocol；speclink show add-change-plan-remote --json 的 deltaSpecs 含 client-protocol/spec.md → 假設清單該條標為「Covered by canon（進行中變更 add-change-plan-remote 將改動：<該 delta 的 Requirement 名>）」；Context 的相關變更句含 `add-change-plan-remote: client-protocol`。
...
**失敗模式**：技能文字的指示不產生執行期錯誤；正式規格零命中、沒有進行中變更、show --json 的 deltaSpecs 為空三種情況都是靜默跳過，不提示、不阻斷；remote 模式走同一組 Dual 動詞，行為與 fs 模式一致。
...
- **show --json 讀量**：payload 帶三份 artifact 全文，代理人只取 deltaSpecs 欄位；技能文字明講「只用該欄位」，避免偵察被拖成調查。
- **跨平台**：不涉及路徑比對與 OS 路徑拼接，動詞輸出三平台同形。
- **remote 模式**：show 與 artifact cat 皆為 Dual 動詞，deltaSpecs 來自 protocol 的 deltaCapabilities，行為與 fs 模式一致。
