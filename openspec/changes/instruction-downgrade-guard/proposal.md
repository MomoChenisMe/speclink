...
- **CLI update 降級守門**（speclink-cli）：update 執行前探測，整體判「較新」時拒絕執行——單行說明含工作區版號與引擎版號、非零 exit code、零寫入；新增旗標 --allow-downgrade 明示越過（不共用 --force，避免慣性帶旗標把守門靜默穿透）。缺失、過期、現版的行為不變。
...
- `workspace-tools`: 「指令檔過期探測」由四態改五態並加方向判定與聚合優先序；新增「update 降級守門」與「引擎版號查詢面」兩條需求。
