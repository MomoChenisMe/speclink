...
repo SHALL 提供本機建置安裝入口（node scripts/desktop-install.mjs），把桌面 app 的本機建置與安裝收成單一指令並以版號斷言取代信任。執行時 SHALL 依序：(1) 印出當前 HEAD、分支、工作樹是否乾淨與源碼的產物層版號；(2) 執行 sidecar 佈署（永遠重新建置，SHALL NOT 沿用 src-tauri/binaries/ 既有檔案）；(3) 前端建置與 tauri bundle——簽章環境變數缺失時 SHALL 單行錯誤停止、不進入建置；(4) 斷言 bundle 內 sidecar CLI 的引擎版號等於源碼產物層版號，不等時 SHALL 印出兩邊版號並以非零結束。帶 `--install` 時 SHALL 續行：(5) 確認 app 未執行——執行中 SHALL 單行錯誤停止，SHALL NOT 代為結束程序；(6) 覆蓋 /Applications 安裝；(7) 斷言安裝版 CLI 的引擎版號同版，不等時 SHALL 印出兩邊版號並以非零結束。安裝步驟（5–7）僅支援 macOS，非 macOS 帶 `--install` SHALL 單行錯誤；建置步驟（1–4）平台中立。任一步驟失敗 SHALL 以非零結束且 SHALL NOT 繼續後續步驟。
...
- **THEN** 單行錯誤指出缺失的變數名、exit code 非零、不進入建置
