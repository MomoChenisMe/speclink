...
activateTab 本地路徑進 probe 前設 pendingTabKey 為目標分頁 key；enterProject 完成（activeKey 翻轉）或 probe 失敗（寫入 tabErrors）時清為 null。主視窗 ProjectTabs 與 tray 面板分頁條對 key 相符的分頁渲染小 spinner。remote 分頁不設：既有 session 直翻無 probe 段、重連走 recovery 狀態機。
替代方案：樂觀翻頁——被討論淘汰（回滾複雜）；只改游標樣式——tray 面板與主視窗分屬不同視窗，游標回饋不可靠也不指向目標分頁。

**D2. B 段 skeleton 條件＝既有 loaded 旗標，僅首訪出現**
看板與 tray 面板分區在活躍快照 loaded 為 false 時渲染 skeleton；loaded 為 true（含舊快取）照常渲染資料，refresh 完成靜默換新。不新增平行 isLoading 狀態——watcher 與切換共用 refresh，平行旗標會造成每次刷新閃爍與語意重疊。
替代方案：以 refresh 進行中為條件——被淘汰（刷新頻繁、閃爍）；世代計數判斷「切換後首輪」——過度設計，loaded 已足。
...
TraySnapshot 新增 pendingTabKey（string | null）與 workspaceLoaded（boolean，活躍快照的 loaded）兩欄，由 buildTraySnapshot 自 store state 導出；面板據以渲染分頁 spinner 與分區 skeleton。面板不自建任何載入狀態，維持薄渲染原則。既有 hideWorkspaceData（remote recovery 遮資料）優先於 skeleton——遮資料時不出骨架。
...
- 點擊主視窗分頁列或 tray 面板分頁條上的非活躍本地分頁：目標分頁立即（同一事件迴圈批次內）出現 spinner；probe 成功翻頁後 spinner 消失；probe 失敗時 spinner 消失且分頁錯誤呈現照舊。
...
- store（apps/desktop/src/store.ts）：AppState 新增 pendingTabKey: string | null，初值 null；僅 activateTab 本地路徑寫入。
- TraySnapshot（apps/desktop/src/tray.ts）：新增 pendingTabKey 與 workspaceLoaded 欄位；panel 經既有 tray-snapshot 事件接收，無新事件、無新 IPC command。
- packages/ui 新增匯出：Skeleton 基元、看板佔位卡、面板佔位列、DocSkeleton。既有元件 props 僅允許新增可選項，不做破壞性變更。

**Verification**

- 單元測試（vitest）：store 的 pendingTabKey 生命週期（設定→翻頁清除；設定→失敗清除）；buildTraySnapshot 兩新欄位導出；KanbanBoard 於 loaded 為 false 渲染 skeleton、true 渲染資料；四抽屜各文件分頁三態（undefined→skeleton、null→空態文案、字串→內容）；TrayPanel 分頁 spinner 與分區 skeleton。
