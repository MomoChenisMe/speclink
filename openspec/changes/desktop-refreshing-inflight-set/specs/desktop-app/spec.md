...
活躍 workspace 尚無已載入快照（首訪）且整批載入以失敗收場時，看板卡片區 SHALL 顯示載入失敗提示文案，SHALL NOT 顯示與真空 workspace 相同的空態文案，SHALL NOT 停留在 skeleton。提示 SHALL 於下一次成功載入後消失並恢復正常呈現。失敗記錄 SHALL 隨 workspace 快照存續——切走再切回仍顯示失敗提示而非骨架或空態。已有舊快取的 workspace 重載失敗 SHALL 維持既有行為（沿用最後成功快照靜默呈現，不顯示失敗提示）。
...
- **THEN** 仍顯示失敗提示，不重新出現 skeleton、不顯示空態文案
