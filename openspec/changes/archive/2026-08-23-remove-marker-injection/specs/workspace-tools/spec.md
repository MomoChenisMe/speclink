...
<!-- REMOVED-SCENARIO: 自訂描述子的指令檔同受守門 -->
...
**Reason**: 指令檔 marker 區塊不再生成，以標記版號為基準的探測失去對象；過期探測改以技能檔 frontmatter 版本欄位為基準（見 ADDED「技能檔過期探測」）。
**Migration**: 探測消費端（CLI update 判定與桌面過期提示）改讀技能檔版本；「指令檔存在但無標記＝退出受管」情境無對應概念——技能目錄整組缺少即判缺失。

### Requirement: marker 技能指引跟隨 worktree 政策

**Reason**: marker 區塊整體停止生成，區塊內 worktree 指引隨之消失；worktree 指引改由 worktree 技能自身（生成集合仍受政策閘，見「worktree 技能的政策條件式生成」）與其交棒句承載。
**Migration**: 政策開關的可見效果只剩技能集合的增減；品質站於 worktree 內、封存僅主 checkout 的指引由 skill-routing 的出口交棒句與引擎拒絕行為承載。
