...
- 相容性影響：wire contract 僅 additive——TaskDoneRequest 早已攜 touchedFiles（server 開始消費），並新增選填 headCommit（品質站對齊 spec「headCommit 由 wire 攜入」的結論；缺席即缺席、不偽造）；drift 回應的 change 輸入集新增選填 evidence（store 記錄隨回應送到 checkout 側，remote drift 的 Environment 維度才有 touched 事實）。task-completed 事件 payload 增欄位為 additive，既有訂閱者無需遷移；task done 的人眼輸出與 --json 皆不變，無回歸對照破壞；本地 .evidence.json 檔案格式與位置不變。
