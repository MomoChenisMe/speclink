...
**介面**：`init`、`init_remote`、`probe_assets` 簽名不變；`SyncPlan::write_skills` 刪除；`SyncPlan::differing_files` 簽名不變、語意改為全部 target；`AssetProbe`／`ToolAssetState` serde 形狀不變。連帶移除本變更孤兒化的 crate 私有項目：`SyncTargetKind` enum、`SyncTarget.kind`（探測與差異比對不再依 target 種類分流後無讀取者）與 `SyncPlan.worktree_on`（只有 `write_skills` 讀過）——無外部形狀影響。描述子 target 的 `skills_dir` 在系統邊界 `ToolDescriptor::validate` 正規化一次（削去結尾 `/`），並在同處新增兩條拒絕：削完等於專案根者（只由分隔符或 `.` 構成——生成物會散進專案根、清理會掃到根目錄），以及等於內建工具 skills 目錄者（兩個 target 指同一目錄時，探測對同一份 SKILL.md 比兩種期望內容而永遠回報過期，生成時後手的 for_codex 子集也會把前手剛寫的 claude 專屬技能當成孤兒刪掉）。`SyncPlan::resolve` 因此直接信任欄位、不再做字串處理。`ToolSelection::resolve` 在第一個壞描述子之後仍收下後續合法描述子（第一個錯誤保留給 update）。
...
**驗收**：
- 新增測試（`crates/speclink-core/src/init.rs`）：`init_force_switching_tools_prunes_the_deselected_footprint`（claude → codex）、`init_force_prunes_a_renamed_skill_directory`（`speclink-onboard` 殘留）、`init_force_resets_custom_footprint_state`（先 update 生成描述子足跡，`init --force --tools claude` 後 `.speclink/generated-tools.yaml` 不存在且描述子目錄被清）、`init_strips_the_unselected_tools_legacy_marker`（tools=[codex] 的 `init --force`，`CLAUDE.md` 遺留區塊被剝）、`init_over_residual_skill_files_rewrites_them`（無 `.speclink.yaml` 但 `.claude/skills/speclink-apply/SKILL.md` 為舊內容，不帶 force 的 init 後為現版內容）、`probe_reports_missing_for_a_descriptor_without_skills`、`probe_lists_descriptor_paths_in_differing_files`（描述子落後與兩側同時落後合一）、`probe_ignores_an_invalid_descriptor`、`probe_still_covers_a_valid_descriptor_after_an_invalid_one`、`probe_normalizes_a_descriptor_skills_dir_with_a_trailing_slash`、`tool_selection_keeps_valid_descriptors_after_a_bad_one`；`crates/speclink-core/src/config.rs`：`descriptor_validation_normalizes_a_trailing_slash`、`descriptor_validation_rejects_a_skills_dir_that_normalizes_to_nothing`、`descriptor_validation_rejects_a_builtin_skills_dir`；`crates/speclink-cli/tests/it/init_tools.rs`：`force_reinit_with_a_new_selection_prunes_the_old_tool_and_keeps_the_two_line_summary`、`update_rejects_a_descriptor_skills_dir_at_the_root_or_a_builtin_directory`。
...
**範圍邊界**：in scope＝`crates/speclink-core/src/init.rs` 的 init／init_remote／differing_files／probe_assets 與測試；`crates/speclink-core/src/config.rs` 的 `ToolDescriptor::validate`（skills_dir 正規化與兩條拒絕）與其測試；`crates/speclink-cli/tests/it/init_tools.rs` 的兩個 CLI 測試；workspace-tools 的三條 delta。out of scope＝CLI 輸出、desktop 前端、`.speclink.yaml` 重寫語意、`CustomFootprint` 格式、golden、`ASSET_VERSION`。
...
- [跨平台：描述子 `skills_dir` 的拼法差異] → 正規化落在 `ToolDescriptor::validate`（只削結尾 `/`），所有消費端讀同一份削過的字串，回報路徑與磁碟路徑同源；反斜線不特別處理——在 Unix 它是合法檔名字元，改寫它反而讓兩者分岔。`differing_files` 讀檔用 `skills_root.join(dir)`（PathBuf）；既有 `probe_ignores_line_ending_differences` 守住 CRLF。
- [舊足跡記著未削的 `skills_dir`] → 升級後第一次同步判它已下架、先剪再重寫同一目錄，`pruned` 多報一次該工具；無資料損失（剪在寫之前），之後兩邊同形即消失。
