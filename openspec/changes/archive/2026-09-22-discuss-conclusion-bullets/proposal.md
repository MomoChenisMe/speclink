...
- 第 8 條同時規定多刀結論的固定寫法：每刀一個 bullet，開頭 `**cut N \`change-name\`**：一句範圍`，細節縮排子項、一子項一件事；Decision 保留全部定案細節、不縮水、各點（含縮排子項）不回指輪次。
- 第 8 條規定 Rejected alternatives 每項一行「方案——落敗理由」；Deferred 每項一行「問題——為何現在不解」或單寫 none。
- 技能檔的 conclude 模板（`speclink discuss conclude … --stdin` 範例）與「Capture decisions」的 Conclusion 摘要格式同步改為條列形（兩份模板的提示字句同文、Deferred 行示範 `- ` 形狀），讓 agent 照抄模板就符合第 8 條。
- speclink-improve 技能（crates/engine/speclink-core/assets/skills/improve.md）是 discuss 的鏡像入口、寫入同一種討論記錄：其兩個 conclude 範例（收斂轉出、全數否決）同步改為條列形——收斂範例 Decision 後接 `- ` 與多刀 bullet，全數否決範例 Decision 維持一句「不做」定論、Rejected alternatives 條列。
- ASSET_VERSION（crates/engine/speclink-core/src/workspace/init.rs）自 v1.39.0 升至 v1.40.0；五份 render golden 與 assets.lock 於乾淨樹重生；`speclink update` 重生 .claude 與 .agents 下的 speclink-discuss 與 speclink-improve SKILL.md（其餘 33 份技能實例只有 version 行變動）。
- 正式規格 discuss-skill 的「討論記錄的樹慣例與格式不變」MODIFIED：補上結論欄位的條列規則與多刀寫法，並維持「既有記錄不需遷移」的保證；improve-skill ADDED 一條「conclude 範例與討論結論條列規則同形」需求，釘住兩個範例的形狀。
- 影響的技能與工具：speclink-discuss 與 speclink-improve 兩支，claude／codex／neutral 三種渲染目標同源；引擎動詞、CLI 旗標、`--json` 輸出、config 欄位皆不變，無相容性影響；既有討論記錄不回改，舊格式結論在桌面仍走「討論結論以欄位標籤呈現」的既有渲染。
...
(none)——規格掃描命中 discuss-skill（技能訪談行為與記錄格式）、discussion-docs（記錄檔名與鏈結語意）、desktop-app（結論分頁渲染）；本變更只改技能文字規則與範例：discuss 的規則落在 discuss-skill 既有的「討論記錄的樹慣例與格式不變」需求內，improve 的範例在既有 improve-skill capability 下新增一條需求，都不需新 capability。
...
- `improve-skill`: 新增「conclude 範例與討論結論條列規則同形」需求——兩個 conclude 範例改為條列形；六步骨架、掃描精髓與 candidates 記錄等既有需求不變。

## Impact

- Affected specs: discuss-skill（MODIFIED）、improve-skill（ADDED）
- Affected code:
  - New: (none)
  - Modified:
    - crates/engine/speclink-core/assets/skills/discuss.md（Document rules 第 8 條、conclude 模板範例、Capture decisions 摘要格式）
    - crates/engine/speclink-core/assets/skills/improve.md（收斂轉出與全數否決兩個 conclude 範例）
    ...
    - .claude/skills/speclink-discuss/SKILL.md、.agents/skills/speclink-discuss/SKILL.md、.claude/skills/speclink-improve/SKILL.md、.agents/skills/speclink-improve/SKILL.md（speclink update 重生；其餘 33 份 SKILL.md 僅 version 行變動）
  - Removed: (none)
