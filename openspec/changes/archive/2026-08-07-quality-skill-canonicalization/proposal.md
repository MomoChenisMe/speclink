...
- **新增正典技能 quality**：crates/speclink-core 新增技能 asset（延後蓋章的兩站時序編排：review 檢查先不蓋章 → verify 檢查先不蓋章 → 兩站 findings 統一修正 → 各自複驗至必修淨空、仍不蓋章 → 一輪完整複驗零新修正後兩章接連補蓋 → 封存），註冊進 skills.rs 技能表，`speclink update` 生成 speclink-quality 技能檔至已啟用工具（claude／codex），golden 對照涵蓋。
- **兩站正典補 quality 時序例外**：review 與 verify 技能 asset 各補「quality 時序中零 findings 的 discovery 與必修淨空的 validation 輪皆不自動蓋章、改走先不蓋章出口，蓋章延至 quality 收尾補蓋（補蓋前內容又移動則先驗後蓋）」——堵掉「乾淨首輪自動蓋章」與「複驗淨空即蓋」在 quality 時序下破壞「兩章皆過才蓋」保證的縫隙（cross-station-staleness 原記為 Deferred，本次收斂）。
- **生成文件範本更新**：init.rs 範本的主 workflow 行與 worktree 流程行加入 quality 入口、技能使用清單加入 quality 條目（觸發時機：兩站都跑時；單站直接呼叫該站技能）；CLAUDE.md／AGENTS.md 經 `speclink update` 刷新，不手改生成物。
- **README 說明文件**：README.md／README.en.md 兩站分工表補「兩站都跑 → /speclink-quality」入口與時序說明，SDD workflow 流程圖主流程行同步補 quality 入口。
