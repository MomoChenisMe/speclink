...
<!-- BEFORE: 討論清單的誠實映射條款僅涵蓋 promotedTo，未涵蓋 concluded -->
...
- **THEN** 已轉出者落入已轉出群組且連結其 change 名；未轉出者維持一般群組；無討論被固定空清單錯誤歸組

#### Scenario: 討論 concluded 映射且缺席不補值

- **WHEN** remote 分頁接一台送 concluded 欄位的 server 載入討論清單，再接一台不送該欄位的舊 server 載入同清單
- **THEN** 前者每筆討論的結論狀態如實映射 wire 值；後者以未知呈現（分區退回既有判準），無任何討論被 client 端補成 false
