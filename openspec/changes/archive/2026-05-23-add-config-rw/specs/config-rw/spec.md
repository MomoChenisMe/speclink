...
| `config.key_not_found` | 2 | List the closest matching key paths in the current config (best-effort) |

The error code `state.etag_mismatch` (exit 7) is already registered by earlier slices and SHALL apply to `config.write` without redefinition.
...
#### Scenario: All five codes appear in CLI error registry

- **WHEN** the CLI binary is built with A5 patches
- **THEN** `speclink describe-errors --json` (or the equivalent test harness query) SHALL include `config.not_found`, `config.malformed`, `config.key_not_found`, `config.external_edit_detected`, and `config.malformed_using_defaults` in the registry output
