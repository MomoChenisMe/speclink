...
- [x] 1.1 新增 Tauri command `hide_tray_panel`（設計 D2）：apps/desktop/src-tauri/src/lib.rs 單行委派到 apps/desktop/src-tauri/src/panel.rs 的 hide 函式；面板不存在或已隱藏時無事、非 macOS 無事（命令不會失敗，無回傳值）。驗證：`cargo check -p speclink-desktop` 通過，且 lib.rs 的 generate_handler 列表含 hide_tray_panel。 <!-- speclink-task:tsk_01M2MKHFMKA3TD9V7WANJBKNWN -->
