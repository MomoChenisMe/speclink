...
<!-- BEFORE: 兩站檢查後自動統一修正、自動複驗至淨空、零新修正後自動接連補蓋兩章並建議封存，全程不徵詢使用者 -->
...
- 收尾：使用者裁示補蓋後，review 章與 verify 章接連補蓋；補蓋時技能 SHALL 向兩站明示本次為收尾補蓋呼叫——兩站據此關閉其禁蓋例外；未宣告即補蓋不合本規格。兩章 SHALL 接連落、中間零編輯，使兩章至封存皆不出現「其後有變動」狀態。封存 SHALL 以建議形式提出、由使用者執行。
