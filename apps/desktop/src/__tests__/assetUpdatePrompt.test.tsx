// 技能檔提示橫幅（desktop-app spec 過期提示，決策 7）：分頁內容頂部
...
describe("AssetUpdatePrompt", () => {
  ...
  it("缺失態：主動作為「安裝」（從未安裝的專案不以更新稱之）", () => {
    const h = handlers();
    render(<AssetUpdatePrompt prompt={MISSING} error={null} busy={false} {...h} />);
  ...
  it("較新態：以「app 是舊版」語意呈現，只留保留現狀、無任何改寫動作", () => {
    const h = handlers();
    render(<AssetUpdatePrompt prompt={NEWER} error={null} busy={false} {...h} />);
  ...
  it("進行中：主動作停用，避免重複觸發", () => {
    const h = handlers();
    render(<AssetUpdatePrompt prompt={STALE} error={null} busy {...h} />);
  ...
  it("底色中性、狀態由圖示語意色承載：過期為琥珀警示、套用失敗為紅", () => {
    ...
    const { unmount } = render(
      <AssetUpdatePrompt prompt={STALE} error={null} busy={false} {...h} />,
    );
    const stale = screen.getByTestId("asset-prompt");
    ...
    render(<AssetUpdatePrompt prompt={STALE} error={"寫入失敗"} busy={false} {...h} />);
    // 套用失敗是錯誤，不是警示——與過期提示分色才看得出嚴重度差別。
    const failed = screen.getByTestId("asset-prompt");
  ...
  it("無提示時不渲染任何內容", () => {
    const h = handlers();
    render(<AssetUpdatePrompt prompt={null} error={null} busy={false} {...h} />);
