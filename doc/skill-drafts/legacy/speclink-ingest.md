...
description: "Update an existing SpecLink change from new context (discussion / plan / conversation)"
effort: xhigh
...
1. **Linked discussion** — a discussion with `status: converged` and `Capture to: ingest <change>` matching the target change. This is the cleanest source because the discussion already structured the change reasoning.
...
**Input**: Optionally specify the change name. Examples:
...
1. **Locate the source context**

   a. **`--from-discussion <topic-id>` provided** → look up the discussion:

   ```bash
   speclink discuss show <topic-id> --json
   ```

   - If found and `status == "converged"` → use its content as the source. Proceed to Step 2.
   - If `status == "iterating"` → STOP. Tell the user the discussion hasn't converged yet; finish it first via `/speclink-discuss <topic-id>` then run `discuss conclude`.
   - If not found → report error and STOP.

   b. **No `--from-discussion`, check for matching converged discussions**:

   ```bash
   speclink discuss list --status converged --about <change-name> --json
   ```
   ...
   c. **Plan file (if your AI tool supports one)**:

   - Check conversation context for a plan file path. If found and accessible, use it.
   ...
   d. **Conversation context fallback**:
...
2. **Parse the source structure** (skip if conversation context)
...
4. **Select the change and check state**
   ...
   **State handling** (ingest is valid from multiple states, but with different implications):
   ...
   - `state: "code_reviewing"` → unusual; normally rejection auto-retreats to `in_progress`. If somehow ingest is needed at code_reviewing, ask user to reject + re-shape via the normal feedback loop instead.
   - `state: "archived"` → STOP. Cannot ingest into an archived change.

   **Parked handling**:

   - If `parked: true`, inform the user and ask: continue (unpark and proceed) or cancel.
   - If continue: `speclink unpark "<name>"`, then proceed.
   ...
   **Read existing artifacts for context** before updating:
   ...
   ```bash
   speclink show change "<name>" --json
   ```

5. **Update artifacts**

   For each artifact you intend to update, get instructions first:
   ...
   The instructions JSON includes `locale` — the language to write artifacts in. **Exception**: spec files (`.speclink/changes/<name>/specs/**/*.md`) MUST always be written in English regardless of locale, because they use normative language (SHALL/MUST).
   ...
   **Parallel task markers (`[P]`)**: When updating tasks.md, read `.speclink/config.yaml#parallel_tasks` (instruction JSON exposes this). If `true`, add `[P]` markers to **new** tasks that qualify. If `false`, do NOT add new `[P]` markers — but **preserve any existing `[P]` markers already in the file**.

   Write each updated artifact via CLI (the `--overwrite` flag is required when rewriting an existing artifact body):

   For **proposal / design / tasks** (single-instance):

   ```bash
   speclink new artifact <artifact-id> --change "<name>" --stdin --overwrite <<'ARTIFACT_EOF'
   <content>
   ARTIFACT_EOF
   ```

   For **spec** (one command per capability):

   ```bash
   speclink new artifact spec --change "<name>" --capability <capability-name> --stdin --overwrite <<'ARTIFACT_EOF'
   <delta spec content>
   ARTIFACT_EOF
   ```
   ...
   After writing each artifact, re-check status:
...
6. **Inline Self-Review** (before letting the engine auto-validate)
...
7. **Analyze-Fix Loop** (max 2 iterations)
   ...
   Engine auto-validates each `new artifact --overwrite` write. After all artifacts are updated, run cross-artifact analysis:
   ...
   3. If Critical/Warning findings exist:
      ...
      b. Fix each finding in the affected artifact (`new artifact <kind> --overwrite`).
...
9. **Summary and next steps**
   ...
   - `state: "reviewing"` → "Reviewer should re-approve via `speclink review approve --change <name> --phase artifact`."
...
- `change.locked` (another agent is writing) → retry 1-2 sec × max 3 attempts.
- `change.not_found` → suggest `speclink list --changes --json` (with `--parked` for parked).
- `discussion.not_found` (when `--from-discussion` doesn't match) → suggest `speclink discuss list --status converged --json`.
- `discussion.iterating` (when the linked discussion hasn't converged) → STOP; suggest finishing the discussion first.
- `state.transition_invalid` → ingest is not valid at this state (e.g., archived); follow Step 4 state table.
- `artifact.validation_failed` → engine returns specific errors; fix content and retry.
...
- **NEVER** write application code — this skill only creates/updates SpecLink artifacts via CLI.
...
- **DOES**:
  - Update artifacts in place with `--overwrite`.
