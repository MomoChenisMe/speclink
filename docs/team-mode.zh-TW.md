...
`.speclink.yaml` 的一個區段決定模式：

| repo 的狀態 | 模式 |
|---|---|
| `.speclink.yaml` 無 `remote:` 區段 | **fs**——經典本地 `openspec/` 佈局，一切照舊 |
| 有 `remote:` 區段 | **remote**——動詞呼叫團隊系統的契約端點 |
| `remote:` 區段與 `openspec/` 並存 | **remote 勝出**；每個指令在 stderr 印一行並存警告（通常是遷移殘留——移除其中一邊） |

區段只有兩個欄位：

```yaml
# .speclink.yaml — committed，如 .lfsconfig：每個 clone 拿到同一份綁定
tools:
  - claude
remote:
  url: https://team.example.com/api/speclink/v1/projects/erp   # 含專案範疇；可缺省改由 SPECLINK_STORE_URL 供給
  repo: backend    # 單 repo 專案可缺省——本 repo 在專案內的註冊名
```

- `url` 攜帶**專案範疇**——一台團隊 server 可承載多個專案；你的 repo 恰綁定一個。
- `SPECLINK_STORE_URL` 為單一 shell 或 CI job 覆寫（或供給）url（staging server、port-forward，或團隊不想 commit 的 url）。它絕不會把 fs 工作區變成 remote——區段本身才是模式訊號。
- 區段存在但**兩處都沒有 url**（`remote.url` 與 `SPECLINK_STORE_URL` 皆缺）是明確失敗，訊息同時指名兩種設定方式——CLI 絕不靜默退回 fs 模式。
- 憑證永不放在這個檔案（也不放在 repo 內任何地方）。

### 自 `.speclink.remote.yaml` 遷移

較早的版本以獨立連接檔 `.speclink.remote.yaml` 承載連接設定。該檔已不再被讀取：殘留檔案會讓每個指令在 stderr 印一行遷移警告，且不影響模式判定——未遷移的專案會以 fs 模式執行（無 `openspec/` 樹時明確報錯）。遷移方式：把 `url` 與 `repo` 兩個值搬入 `.speclink.yaml` 的 `remote:` 區段，然後刪除舊檔。

注意：`init --store remote`、`link`、`unlink` 重寫 `.speclink.yaml` 時保留每個欄位的**值**，但 YAML 註解不會在重寫後存活。
...
只執行 **workspace init**：`CLAUDE.md`／`AGENTS.md` marker 區塊（remote 措辭）、技能、`.claude/settings.json`、`.gitignore` 條目、含 `remote:` 區段的 `.speclink.yaml`。刻意**不**建立 `openspec/` 樹——文件活在 server 上。

### 既有 repo：`speclink link`／`speclink unlink`

```bash
speclink link https://team.example.com/api/speclink/v1/projects/erp --repo backend
speclink unlink   # 移除 remote 區段；回到 fs 模式
```

`link` 寫入（或更新）`.speclink.yaml` 的 `remote:` 區段並保留其他欄位——既有的 `tools:` 清單原封不動。`unlink` 只移除區段，不刪檔案。已登入時 `link` 立即驗證（見下）；未登入時照樣寫入區段並提醒執行 `speclink auth login`——離線 link 絕不阻塞，改由第一個動詞驗證。
...
2. **Repo 身分**：`remote.repo` 欄位是本 repo 在專案 repos 註冊表（server 端管理）的註冊名。`init`／`link` 在有憑證時立即驗證——名字不在註冊表即大聲失敗並列出可用名單，且不寫入區段。單 repo 專案可缺省，由 server 解析。
...
| `repo is not registered in this project (available: …)` | `remote.repo` 不在註冊表 | 修正 `remote.repo` 或重跑 `speclink link` |
...
| `server unreachable — check the connection url (\`remote.url\` in .speclink.yaml or SPECLINK_STORE_URL)` | 連不上 | 修連線；**沒有離線模式、沒有快取答案** |
...
- `.speclink.yaml`（tools 清單、workspace 選項，以及 `remote:` 連接區段本身）——工作區仍自行決定生成哪些 AI harness 檔案。
