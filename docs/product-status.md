...
| Verify and task evidence | Partial（部分可用） | `speclink task done`, local evidence, `validate`/`analyze`; the engine contains a verify workflow asset | [Host evidence implementation](../crates/speclink-host/src/evidence.rs)<br>[Phase 2 chain test](../crates/speclink-server/tests/it/phase2_chain.rs) | This repository has no generated `$speclink-verify`. The server currently discards touched files from remote task completion; an ignored defect case preserves the gap. | 2026-07-17 |
