...
Last audited: **2026-08-13**. This document is the canon for "can I use this yet". The specs under `openspec/specs/` are the canon for behavior and boundaries, and the [Project Roadmap](roadmap.md) describes the direction that matters to users.
...
| Desktop board and drawer | Available | Partial | Connections, sign-in, and the Server list work; the construction path for a full Remote Workspace is not closed yet. |
| Touched-file evidence for tasks | Available | Partial | Local writes `.evidence.json` in the change directory; the server currently discards touched files from remote tasks. |
...
| Node N-API SDK | Partial | Build `crates/speclink-node` from this repo and load it by path | [Node package entry](../crates/speclink-node/package.json)<br>[dispatch contract tests](../crates/speclink-node/__test__/dispatch-contract.spec.ts) | **Not published to npm**: `npm install @speclink/engine` resolves nothing, so a repo build (and a Rust toolchain) is currently required. The Engine and Store bridge itself works; the full Node Host and Copilot Tool packages are not delivered. | 2026-08-13 |
...
| Desktop Server Connections | Available | The Server list in Desktop settings, device login, PAT fallback, logout, and the OS keychain | [Tauri connection orchestration](../apps/desktop/src-tauri/src/connections.rs)<br>[Servers panel tests](../apps/desktop/src/__tests__/serversPanel.test.tsx) | Connections and identity are manageable, but signing in does not yet produce a full Remote Workspace. | 2026-08-13 |
| Desktop Remote Workspace | Partial | The remote `WorkspaceLocator` type and the Server connection UI | [Workspace locator](../apps/desktop/src/session.ts)<br>[Workspace-session spec](../openspec/specs/workspace-session/spec.md) | The remote locator has no construction path yet; spec-only, remote-plus-checkout, and offline/conflict sessions await later changes. | 2026-08-13 |
| Remote CLI and Context Projection | Available | `speclink link`, `auth`, `artifact`, and the read-only `.speclink/context/` | [Remote CLI tests](../crates/speclink-cli/tests/it/remote_read_path.rs)<br>[Context materializer](../crates/speclink-host/src/projection.rs) | The current Client Protocol path works; it does not close the Desktop Remote Workspace or remote evidence gaps. | 2026-08-13 |
| Remote task evidence | Partial | Local `speclink task done` writes `.evidence.json`; the same verb works remotely but records no evidence | [Task evidence implementation](../crates/speclink-core/src/tasks.rs)<br>[Defect preserved as an ignored test](../crates/speclink-server/tests/it/phase2_chain.rs) | The server discards touched files from remote tasks; the gap is held open by an `#[ignore]` test pending the `remote-task-evidence` change. | 2026-08-13 |
...
`@speclink/engine` is not published to npm. The documentation demonstrates a repo-built loading path (see [Node SDK](sdk-node.md)); the direction and observable next step for the npm channel are in the [Project Roadmap](roadmap.md).
