...
最後查核日期：**2026-08-13**。本文是「目前能不能用」的正典。行為與邊界的正典則是 `openspec/specs/` 底下的規格，[專案路線圖](roadmap.zh-TW.md)則描述對使用者有意義的方向。
...
| Desktop Server Connections | Available（可用） | Desktop 設定中的 Server 清單、device login、PAT fallback、logout 與 OS Keychain | [Tauri connection orchestration](../apps/desktop/src-tauri/src/connections.rs)<br>[Servers panel tests](../apps/desktop/src/__tests__/serversPanel.test.tsx) | 可管理連線與身分，但登入後尚不能建立完整 Remote Workspace。 | 2026-08-13 |
| Desktop Remote Workspace | Partial（部分可用） | `WorkspaceLocator` remote 型別與 Server connection UI | [Workspace locator](../apps/desktop/src/session.ts)<br>[Workspace-session spec](../openspec/specs/workspace-session/spec.md) | remote locator 目前沒有建構路徑；spec-only、remote+checkout、offline/conflict 工作階段仍待後續變更閉合。 | 2026-08-13 |
...
| Remote task evidence | Partial（部分可用） | 本地 `speclink task done` 落 `.evidence.json`；遠端同動詞可用但不落 evidence | [Task evidence 實作](../crates/speclink-core/src/tasks.rs)<br>[缺陷保留於 ignored 測試](../crates/speclink-server/tests/it/phase2_chain.rs) | server 目前丟棄 remote task 的 touched files，測試以 `#[ignore]` 明確保留缺口，待 `remote-task-evidence` 變更閉合。 | 2026-08-13 |
