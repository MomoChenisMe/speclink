...
| Install channels / 安裝通路 | Available（可用） | 桌面安裝檔（macOS universal dmg、Windows NSIS、Linux AppImage）、CLI 的 npm（`@speclink/cli`）／安裝腳本／Homebrew tap、server 的 npx 與 Docker | [安裝腳本測試](../scripts/install.test.mjs)<br>[Homebrew formula 產生器](../scripts/release/homebrew-formula.mjs) | 桌面與 CLI 三平台皆有通路；Windows 安裝檔目前未經程式碼簽章，首次執行需放行 SmartScreen。 | 2026-09-14 |
