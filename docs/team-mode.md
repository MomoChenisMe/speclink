...
## The connection file and mode resolution

One file decides the mode:

| State of the repo | Mode |
|---|---|
| No `.speclink.remote.yaml` | **fs** — the classic local `openspec/` layout; nothing changes |
| `.speclink.remote.yaml` present | **remote** — verbs call the team system's contract endpoints |
| Both the connection file and `openspec/` present | **remote wins**; each command prints one stderr warning about the coexistence (typically migration leftovers — remove one side) |

The file has two fields:

```yaml
# .speclink.remote.yaml — committed, like .lfsconfig: every clone gets the same binding
url: https://team.example.com/api/speclink/v1/projects/erp   # required, project-scoped
...
- `SPECLINK_STORE_URL` overrides the url for one shell or one CI job (staging server, port-forward). It only overrides the url of an existing connection — it never turns an fs workspace into a remote one.
...
This performs the **workspace init only**: the `CLAUDE.md`/`AGENTS.md` marker block (in its remote wording), the skills, `.claude/settings.json`, `.gitignore` entries, and the connection file. It deliberately does **not** create an `openspec/` tree — documents live on the server.
...
speclink unlink   # removes the connection file; back to fs mode
```

`link` writes the connection file. When you are already logged in, it validates immediately (see below); when you are not, it writes the file and reminds you to run `speclink auth login` — offline linking never blocks, the first verb validates instead.
...
2. **Repo identity**: the `repo:` field is this repo's registered name in the project's repos registry (managed server-side). `init`/`link` validate it immediately when credentials exist — an unknown name fails loudly and lists the registered names, and no connection file is written. Single-repo projects may omit `repo:`; the server resolves it.
...
| `repo is not registered in this project (available: …)` | `repo:` names something the registry doesn't know | fix `repo:` or re-run `speclink link` |
...
| `server unreachable — check the connection url in .speclink.remote.yaml (or SPECLINK_STORE_URL)` | can't connect | fix connectivity; there is **no offline mode and no cached answer** |
...
- `.speclink.yaml` (tools list, workspace options) — the workspace still decides which AI harness files are generated.
