...
- `npm ci` 可在 `crates/adapters/speclink-node` 成功。
...
- `npm ci` 因 `crates/adapters/speclink-node/package.json` 與 lock file 的 platform optional dependencies 不同步而失敗；現有 Node CI 也使用 `npm ci`，因此 G0 必須先修。
