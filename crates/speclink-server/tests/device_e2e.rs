...
fn session_cookie(response: &ureq::Response) -> String {
    response
        .header("set-cookie")
...
/// Walk the browser-shaped path generated by Desktop when no server session
/// exists: log in over the browser JSON API carrying the device code (the Server
/// returns the activation destination), then approve through the activation API.
fn login_and_approve_through_activation(base: &str, user_code: &str) -> String {
    let login = agent()
        .post(&format!("{base}/api/speclink/v1/web/login"))
        .send_json(serde_json::json!({ "email": EMAIL, "password": PASSWORD, "userCode": user_code }))
        .expect("login");
    let cookie = format!("speclink_session={}", session_cookie(&login));
    let body: serde_json::Value = login.into_json().expect("login body");
    assert_eq!(
        body["data"]["destination"],
        serde_json::json!(format!("/activate?user_code={user_code}")),
        "login returns to the one allowed activation destination"
    );

    approve(base, &cookie, user_code);
    cookie
}

/// Approve through the JSON activation API: the no-action confirm step (pending),
/// then the explicit approval.
fn approve(base: &str, cookie: &str, user_code: &str) {
    let confirm = agent()
        .post(&format!("{base}/api/speclink/v1/web/activate"))
        .set("Cookie", cookie)
        .send_json(serde_json::json!({ "userCode": user_code }))
        .expect("confirm step");
    assert_eq!(confirm.status(), 200, "the confirm step succeeds");
    let done = agent()
        .post(&format!("{base}/api/speclink/v1/web/activate"))
        .set("Cookie", cookie)
        .send_json(serde_json::json!({ "userCode": user_code, "action": "approve" }))
        .expect("approve");
...
fn device_flow_end_to_end_over_a_real_sqlite_server() {
    ...
    // 2. Start from Desktop's activation URL with no browser session, return
    //    through login, observe the prefill, and approve explicitly. (The
    //    pending/slow_down cadence is covered in device_flow; a real client
    //    respects the declared interval, so this test polls once after approval.)
    let cookie = login_and_approve_through_activation(&base, &init.user_code);
    ...
    let revoke = agent()
        .post(&format!("{base}/api/speclink/v1/web/account/devices/{}/revoke", fam_b.id))
        .set("Cookie", &cookie)
        .send_json(serde_json::json!({}))
        .expect("revoke family B over the account API");
    assert_eq!(revoke.status(), 200, "the account revoke succeeds");
