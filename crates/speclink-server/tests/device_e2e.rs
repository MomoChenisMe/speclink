...
/// exists: activation query → login → returned prefilled activation page →
/// explicit confirm and approval.
fn login_and_approve_through_activation(base: &str, user_code: &str) -> String {
    let activation = agent()
        .get(&format!("{base}/activate?user_code={user_code}"))
        .call()
        .expect("activation redirects to login");
    assert_eq!(
        activation.header("location"),
        Some(format!("/login?user_code={user_code}").as_str()),
        "the unauthenticated activation preserves its device code"
    );

    let login_location = activation.header("location").expect("login location");
    let login_page = agent()
        .get(&format!("{base}{login_location}"))
        .call()
        .expect("login page")
        .into_string()
        .expect("login body");
    assert!(
        login_page.contains(&format!("type=\"hidden\" name=\"user_code\" value=\"{user_code}\"")),
        "the login form carries the device code: {login_page}"
    );
    ...
    assert_eq!(
        ...
        Some(format!("/activate?user_code={user_code}").as_str()),
    ...
    let entry = agent()
        ...
        .set("Cookie", &cookie)
        .call()
        .expect("prefilled activation page")
        .into_string()
        .expect("activation body");
    assert!(
        entry.contains(&format!("name=\"user_code\" value=\"{user_code}\"")),
        "the returned activation page is prefilled: {entry}"
    );
    assert!(!entry.contains("name=\"action\""), "GET still requires the explicit next step");
