...
1. **Determine the requirement source**
   ...
   - Check if the conversation context mentions a plan file path (plan mode system messages include the path like `{{PLAN_DIR}}<name>.md`)
   - If found, check if the file exists at `{{PLAN_DIR}}`
...
3. **Two or more** → work out an execution order before any apply suggestion:
   - **Hard signal — delta capability overlap**: two changes that both carry a delta for the same capability (the same directory name under `{{SPEC_DIR}}changes/<name>/specs/`) must run sequentially — their deltas rewrite the same canonical spec, and archiving them out of order can trip the merge gate.
