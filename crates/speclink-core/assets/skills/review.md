...
3. **Check the ticket, then resolve the frozen scope**
   ...
   - **Ticket exists and `lastRound.findings` is empty** (e.g. a refused stamp left a clean round behind) → do NOT re-review: once the external gate recovers, retry the stamp directly — `speclink review stamp "<name>" --agent {{TOOL}}` — and report the outcome. No new discovery, no new validation.
...
9. **Branch on the blocking set**
   ...
   - **Bn is empty but accepted findings remain** → recommend the user explicitly stamp with reservations — `speclink review stamp "<name>" --accept --agent {{TOOL}}` — and report **passed with reservations**. Never run `--accept` unprompted.
