Generate a human-readable operating manual from the canonical specs, or walk the user through the system in conversation. The manual is a wiki-style set of Markdown pages under `{{SPEC_DIR}}manual/`, written for someone who joined today and only wants to know how to operate the system.

**Input**: Optional arguments after `/speclink:manual`. They pick the mode:

| Arguments | Mode |
| --- | --- |
| none | **Generation** (the default) — read the specs, write `{{SPEC_DIR}}manual/*.md` |
...
If `{{SPEC_DIR}}manual/` exists, read the frontmatter of every `.md` page in it before anything else. Keep, per page: filename, `title`, `section`, `order`, `sources`, `generated`. This index drives the staleness report (Step 3) and the order-preservation rule (Step 5).
...
| **orphan page** | a page with a non-empty `sources` whose capabilities have all disappeared from `{{SPEC_DIR}}specs/` — reported, NEVER deleted |
...
**Location and filename**: every page is a file directly under `{{SPEC_DIR}}manual/` (create the directory when missing). Filenames are kebab-case ASCII ending in `.md` — `first-login.md`; never spaces, capitals, or non-ASCII. Nothing is written anywhere else.
...
- `about.md` — `title` is `本手冊的來源`, `order` is the largest in the manual. Body states: the source scope is `{{SPEC_DIR}}specs/`; the journey backbone (transcribed from which specs, or rebuilt); the list of contradictions found between newer and older descriptions inside the specs (write `未發現` when there are none); known limits (no screenshots — the running product wins); and the compilation date.
...
1. **Manual exists** (`{{SPEC_DIR}}manual/` present): read every page's frontmatter as the index.
