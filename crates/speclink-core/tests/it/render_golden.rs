...
/// Collect every generated SKILL.md (registry order) as (relative path, content) pairs.
fn generated_files(root: &Path, skills_dir: &str) -> Vec<(String, String)> {
    let mut out = Vec::new();
...
fn snapshot(root: &Path, skills_dir: &str) -> String {
    generated_files(root, skills_dir)
        .into_iter()
...
fn claude_rendering_is_bit_identical_to_golden() {
    ...
    assert_matches_golden(
        "claude.snapshot.md",
        &snapshot(&root.dir, ".claude/skills"),
    );
}
...
// --- remote init: no instruction file is generated at all ---

/// Spec Scenario「指令檔零受管區塊」in remote mode: the managed set is skills only,
/// so remote init leaves no CLAUDE.md/AGENTS.md behind to steer the agent at local
/// spec paths that don't exist — the verbs carry that guidance instead.
#[test]
fn remote_init_writes_no_instruction_file() {
    let root = TempRoot::new("remote-no-instructions");
    init::init_remote(
        &root.dir,
    ...
    for file in ["CLAUDE.md", "AGENTS.md"] {
        assert!(
            !root.dir.join(file).exists(),
            "{file} must not be generated by remote init"
        );
    }
    for tool in [Tool::Claude, Tool::Codex] {
        assert!(
            root.dir.join(tool.skills_dir()).join("speclink-apply").join("SKILL.md").is_file(),
            "{} skills are still generated",
            tool.name()
        );
    }
}
...
fn neutral_body_has_no_slash_prefix_and_no_plan_mode_references() {
    ...
    for invocation in ["cli", "tool-call"] {
        ...
        for (rel, content) in generated_files(&root.dir, ".wad/skills") {
...
fn neutral_cli_wording_runs_speclink_verbs() {
    ...
    let snap = snapshot(&root.dir, ".wad/skills");
    assert!(
...
// without bumping ASSET_VERSION fails here; the failure message carries the fix.
...
/// out before hashing: the lock tracks asset CONTENT, and a version bump on unchanged
/// content must stay green (spec: 僅遞增版號而 render 內容未變 SHALL 通過).
fn render_fingerprint_input() -> String {
    ...
    normalize_eol(&parts.join("\n")).replace(init::ASSET_VERSION, "{{PRODUCT_VERSION}}")
}
...
fn lock_fix_instructions() -> String {
    format!(
        "fix: bump ASSET_VERSION in crates/speclink-core/src/init.rs (render content changed), \
...
fn embedded_assets_are_locked_to_the_product_version() {
    let current_version = init::ASSET_VERSION;
    ...
    if std::env::var(LOCK_REGEN_ENV).is_ok() {
        ...
        if let Some((locked_version, locked_hash)) = &locked {
            assert!(
                ...
                "refusing to rewrite {ASSETS_LOCK}: render output changed but ASSET_VERSION is \
...
`{LOCK_REGEN_ENV}=1 cargo test -p speclink-core --test it render_golden::`.\n{}",
    ...
    assert_ne!(
        ...
        "embedded asset render output changed while ASSET_VERSION stayed at \
{current_version}.\n{}",
        lock_fix_instructions()
    );
    panic!(
        "ASSET_VERSION moved ({locked_version} → {current_version}) but {ASSETS_LOCK} still \
...
// --- quality skill: generation and the registry-description routing ---

/// Spec「品質關卡技能的生成與正典化」: `speclink update` generates the
/// speclink-quality skill file for both built-in tools, and its entry routing rides
/// the registry description (spec skill-routing), stating the trigger — both
/// stations up front; a single station calls that station directly — plus the
/// per-round pause.
#[test]
fn quality_skill_and_its_routing_are_generated_for_both_tools() {
    ...
    // Entry routing now rides the registry description (spec skill-routing
    // 「入口路由由技能描述承載」): the both-stations trigger and the single-station
    // branch must be readable from the tool's skill list, before the body is opened.
    let description = skills::registry()
        .into_iter()
        .find(|s| s.name == "quality")
        .expect("the quality skill is registered")
        .description;
    for needle in ["both quality stations", "only one station"] {
        assert!(
            description.contains(needle),
            "the quality description must state {needle:?}: {description}"
        );
    }
    // And it must actually reach the generated frontmatter of BOTH tools — a
    // description that never lands in the skill file routes nothing.
    for (rel, content) in skill_for_both_tools("quality-route", "quality") {
        let line = content
            .lines()
            .find(|l| l.starts_with("description:"))
            .unwrap_or_else(|| panic!("{rel}: no frontmatter description"));
        assert!(
            line.contains("both quality stations"),
            "{rel}: the frontmatter description must carry the both-stations trigger: {line}"
        );
    }
}
