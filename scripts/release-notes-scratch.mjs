...
/**
 * 建一個暫存 repo：複製指定腳本到 <dir>/scripts、把 notes 寫進固定的 JSON 路徑
...
export function scratchRepo({ scripts, notes }) {
  ...
  for (const name of scripts) {
    copyFileSync(path.join(ROOT, 'scripts', name), path.join(dir, 'scripts', name));
  }
  writeFileSync(
  ...
  const run = (script, args) =>
    spawnSync(process.execPath, [path.join(dir, 'scripts', script), ...args], { encoding: 'utf8' });
