...
test('test:all 依序納入五個測試面，且 apps/server-web build 排在 cargo test --workspace 之前', () => {
  ...
  assert.match(script, /speclink-node/, 'test:all 缺少 crates/adapters/speclink-node 測試面');
...
test('ci.yml 跑 scripts 測試面，且以 bash 展開 glob 以相容 CI 釘選的 Node 版本', () => {
  ...
  const step = requireIndex(ci, 'node --test scripts/*.test.mjs scripts/*/*.test.mjs', 'ci.yml');
...
test('ci.yml：凡編譯 speclink-desktop 的 job，都在 cargo 之前佈好 CLI sidecar', () => {
  for (const [job, body] of ciJobs()) {
    assertStagedBeforeCargo(
      job,
      body,
      'node scripts/desktop/desktop-sidecar.mjs',
      COMPILES_DESKTOP,
      ' CLI sidecar',
    );
  }
...
test('release.yml 的 release job 以 body_path 前置下載指南，changelog 接續其後', () => {
  ...
  const generate = /node scripts\/release\/release-notes\.mjs --tag [^>\n]+> *(\S+)/.exec(release);
...
test('Dockerfile 以 Node stage 產 dist、Rust stage 內嵌，最終 runtime 僅 server binary＋non-root、無 Node runtime', () => {
  const dockerfile = read('crates/host/speclink-server/Dockerfile');
...
test('node-sdk.yml 可被 release 管線重用，且版號蓋章不經字串內插', () => {
  ...
  const binaryDownload = requireStep(
    pack,
    [/actions\/download-artifact@v4/, /path:\s*crates\/adapters\/speclink-node\/artifacts/],
...
// --- 倉庫路徑守門（delivery-baseline「倉庫路徑守門」） ---

// 掃描面的排除項。前四項是版本控制與建置產物；`.speclink` 與 `.dev` 是本機執行與
// 開發時產生的狀態（前者是凍結快照，內容是歷史事實；後者由 npm run dev 整檔重寫）；`openspec/changes` 底下的提案必然逐字引用
...
/// 逐行產出 repo 內文字檔的 [相對路徑, 行號, 內容]；跳過 @trace 註解區塊（封存
/// 工單留下的歷史檔案清單）與帶豁免標記的行。
function* repoTextLines(dir = root) {
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    const abs = path.join(dir, entry.name);
    const rel = path.relative(root, abs).split(path.sep).join('/');
    if (SCAN_SKIP_PATHS.has(rel)) continue;
    if (entry.isDirectory()) {
      if (!SCAN_SKIP_DIRS.has(entry.name)) yield* repoTextLines(abs);
      continue;
    }
    if (!entry.isFile()) continue;
    const text = readFileSync(abs, 'utf8');
    if (text.includes(NUL)) continue; // 二進位檔
    let inTrace = false;
    let lineNumber = 0;
    for (const line of text.split('\n')) {
      lineNumber += 1;
      if (inTrace) {
        inTrace = !line.includes('-->');
        continue;
      }
      ...
      if (line.includes(PATH_GUARD_ALLOW)) continue;
      yield [rel, lineNumber, line];
    }
  }
}
...
test('repo 不殘留搬移前的舊形路徑（crates 未分組、scripts 未分組、引擎模組未分組、伺服器模組與測試檔未分組）', () => {
  ...
  for (const [rel, lineNumber, line] of repoTextLines()) {
    if (legacyCrate.test(line)) {
      hits.push(`${rel}:${lineNumber} 舊形 crate 路徑（正典為 crates/<group>/speclink-…）\n    ${line.trim()}`);
      continue;
    }
  ...
  assert.equal(
    hits.length,
    0,
    `殘留 ${hits.length} 處搬移前的路徑，逐一改為分組後的路徑；` +
      `只有逐字引用舊路徑當反例的文件行才以 ${PATH_GUARD_ALLOW} 標記豁免：\n${hits.join('\n')}`,
  );
});
